<script>
	...
	const topics = $allCategories.filter((d) => d.id > 10);
	const headingTitle = 'Topics';
	const totalQuantity = $articleList.length;
	const articlesPerTopic = 3;
	const displayQuantity = articlesPerTopic * topics.length;
...
<div class="container">
	<Sidebar allTopics={$allCategories} />
	<div class="posts">
		...
		<div class="resources-container">
			{#each topics as topic}
				<div class="category-container-1">
					<TopicsCard
						articles={filterCategory($articleList, topic)}
						{topic}
